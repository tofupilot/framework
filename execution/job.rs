...
use crate::schema::procedure::{MeasurementSpec, StageScope};
...
pub struct JobResult {
    ...
    pub phase_outcome: Option<Outcome>, // Computed later in completion handler
    pub next_action: Option<crate::schema::procedure::PhaseNextAction>, // Computed in completion handler after applying then config
...
impl JobResult {
    ...
    pub fn should_stop_test(&self) -> bool {
        ...
        if let Some(next_action) = &self.next_action {
            matches!(next_action, crate::schema::procedure::PhaseNextAction::Stop)
        } else {
