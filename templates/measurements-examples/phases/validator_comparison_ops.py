...
def test_comparison_operations(phase, run, ui):
    ...
    measurements.numeric_comparison = voltage
    run.log.info(f"Numeric comparison test: {voltage}V")

    # String comparison (lexicographic)
    versions = ["v0.9.0", "v1.0.0", "v1.1.0", "v2.0.0"]
    version = random.choice(versions)
    measurements.string_comparison = version
    run.log.info(f"String comparison test: {version}")

    # Array unwrapping for comparison - expected [2.0] unwraps to 2.0
    test_voltage = random.uniform(1.0, 3.0)
    measurements.array_unwrap_comparison = test_voltage
    run.log.info(f"Array unwrap comparison test: {test_voltage}V")
