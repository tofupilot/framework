...
def test_edge_cases(phase, test_api, ui):
    ...
    test_api.measurements.empty_values = empty_value
    test_api.log.info(f"Empty values test: {empty_value}")
    ...
    test_api.measurements.complex_values = complex_value
    test_api.log.info(f"Complex values test: {type(complex_value).__name__}")
    ...
    try:
        ...
        test_api.measurements.edge_float_values = edge_float
        test_api.log.info(f"Edge float test: {edge_float}")
    except:
        # Fallback to safe value if there are issues
        test_api.measurements.edge_float_values = 999999.999
        test_api.log.info("Edge float test: large finite value (fallback)")

    # Unknown operators - should result in UNSET outcome
    test_value = 1.0
    test_api.measurements.unknown_operators = test_value
    test_api.log.info(f"Unknown operators test: {test_value}")

    # Boundary conditions - exactly equal to boundary values
    boundary_value = 3.0  # Exactly matches both >= 3.0 and <= 3.0
    test_api.measurements.boundary_conditions = boundary_value
    test_api.log.info(f"Boundary conditions test: {boundary_value}V")
